...
from rlpyt.algos.qpg.ddpg import DDPG, SamplesToBuffer
...
class R2DPG(DDPG):
    ...
    def __init__(
            ...
            replay_buffer_class=None,  # set None to select automatically
        ...
        # self.frame_state_space = frame_state_space
    ...
    def initialize_replay_buffer(self, examples, batch_spec, async_=False):
        ...
        if self.replay_buffer_class is None:
            if self.prioritized_replay:
                ...
                if self.frame_state_space:  # if state space is fram -> like atari
                    ReplayCls = (AsyncPrioritizedSequenceReplayFrameBuffer if async_
                                 else PrioritizedSequenceReplayFrameBuffer)
                else:  # non-frame state space -> vector state space
    ...
    def q_loss(self, samples):
        ...
        delta = y - q
        q_losses = 0.5 * delta ** 2
        abs_delta = abs(delta)
        # NOTE: by default, with R2D1, use squared-error loss, delta_clip=None.
        if self.delta_clip is not None:  # Huber loss.
            b = self.delta_clip * (abs_delta - self.delta_clip / 2)
            losses = torch.where(abs_delta <= self.delta_clip, q_losses, b)
        if self.prioritized_replay:
        ...
        return q_loss, valid_td_abs_errors, priorities
