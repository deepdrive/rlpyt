...
class DeepdriveR2dpgQofMuMlpModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.mlp = MlpModel(input_size=int(input_shape + output_size),
...
class DeepdriveR2dpgMuMlpModel(torch.nn.Module):
    """MLP network feeding into an LSTM and MLP output for Q-value outputs for
    the action set."""
    def __init__(
            self,
    ...
    def forward(self, observation, prev_action, prev_reward, init_rnn_state):
        ...
        mu = self.head(lstm_out.view(T * B, -1))
        mu = self._output_max * torch.tanh(mu)
        mu = restore_leading_dims(mu, lead_dim, T, B)

        # Model should always leave B-dimension in rnn state: [N,B,H].
        next_rnn_state = RnnState(h=hn, c=cn)
        return mu, next_rnn_state
